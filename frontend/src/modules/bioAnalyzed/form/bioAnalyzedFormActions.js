...
const actions = {
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      await BioAnalyzedService.create(values);

      dispatch({
        type: actions.CREATE_SUCCESS,
      });

      Message.success(i18n('entities.bioAnalyzed.create.success'));
  ...
  doUpdate: (id, values) => async (
    ...
    try {
      ...
      await BioAnalyzedService.update(id, values);

      dispatch({
        type: actions.UPDATE_SUCCESS,
      });

      Message.success(i18n('entities.bioAnalyzed.update.success'));
