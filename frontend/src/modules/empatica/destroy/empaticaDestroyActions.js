import listActions from 'modules/empatica/list/empaticaListActions';
...
const actions = {
  ...
  doDestroy: (id) => async (dispatch) => {
    try {
      ...
      await EmpaticaService.destroyAll([id]);

      dispatch({
        type: actions.DESTROY_SUCCESS,
      });

      Message.success(i18n('entities.empatica.destroy.success'));
  ...
  doDestroyAll: (ids) => async (dispatch) => {
    try {
      ...
      await EmpaticaService.destroyAll(ids);
      ...
      Message.success(
        i18n('entities.empatica.destroyAll.success'),
      );
